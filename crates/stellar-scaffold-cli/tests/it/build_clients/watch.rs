...
async fn watch_and_vite_integration_test() {
    TestEnv::from_init("test-project", |env| async {
        Box::pin(async move {
            ...
            assert!(npm_install_output.status.success(), "npm install failed: {}", String::from_utf8_lossy(&npm_install_output.stderr));
            ...
            while (!watch_ready || !vite_ready) && start_time.elapsed() < timeout_duration {
                match tokio::time::timeout(
                    ...
                    Ok(Some((_, line))) => {
                        ...
                        if !watch_ready && line.contains("Watching for changes. Press Ctrl+C to stop.") {
                            watch_ready = true;
                        }

                        // Check for vite ready
                        if !vite_ready && (line.contains("Local:") || line.contains("ready in")) {
                            vite_ready = true;
                        }
            ...
            assert!(watch_ready, "Watch process did not become ready within timeout");
            assert!(vite_ready, "Vite server did not become ready within timeout");
            ...
            // Test HTTP request to vite server to trigger potential errors
            let client = reqwest::Client::new();
            let response = client
                .get("http://localhost:5173/debug/fungible_token_interface_example")
            ...
            for module_path in js_module_paths {
                let _ = client
                    .get(&format!("http://localhost:5173{}", module_path))
                    .timeout(tokio::time::Duration::from_secs(10))
                    .send()
                    .await;
            ...
            assert!(vite_errors.is_empty(), "Vite errors detected during test execution. Errors found: {:?}", vite_errors);
